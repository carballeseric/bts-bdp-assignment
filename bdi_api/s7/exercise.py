...
def get_recommendations(name: str) -> list[dict]:
    ...
    with driver.session() as session:
        ...
        # Friends of friends, not already friends, sorted by mutual friends count
        result = session.run(
            """
            MATCH (p:Person {name: $name})-[:FRIENDS_WITH]-(friend:Person)-[:FRIENDS_WITH]-(fof:Person)
            WHERE fof.name <> $name
            AND NOT (p)-[:FRIENDS_WITH]-(fof)
            RETURN fof.name AS name, fof.city AS city, fof.age AS age, COUNT(friend) AS mutual_friends
            ORDER BY mutual_friends DESC
            """,
            name=name
        )
        recommendations = [
            {"name": r["name"], "city": r["city"], "age": r["age"], "mutual_friends": r["mutual_friends"]}
            for r in result
        ]
    driver.close()
